...
function START_function(start) {
   ...
   let impl_1_2 = 0;
      ...
      if(impl_1_2 == 0){console.log(logic1," subset of ",logic2);}else{
         console.log(logic1," not subset of ",logic2);}
      }  //end of subset
   ...
   let tt = 0;
   let TF_impl_A_B = "T";  //defount values of TFinpl is T 
   let TF_inpl_B_A = "T";
   let proof_right = 0;
   let proof_left = 0;
...
for(let i = 0; i < AR.length; i++){
   ...
   switch(what) {
    ...
    case 9:
      ...
      img = "MMa      i1     and first half of d1";
    break;
    case 10:
      x =  M(AR[i][2], L(AR[i][1], p))
      img = "MLa                 d1 second half"   ;
    break;
    case 11:
      x =  impl(AR[i][0]  , L(AR[i][2], M(AR[i][1], p))  ,  M(AR[i][1], p))  //LM->M 
      img = "LM->M  Axiom M";
    break;
    case 12:
    ...
    case 15:
      x =  impl(AR[i][0]  , L(AR[i][3], L(AR[i][2], p))  ,  M(AR[i][1], p))  //LL->M 
      img = "LL->M  Axiom M";
    break;
    case 16:
      x =  impl(AR[i][0],  M(AR[i][3], p)  , L(AR[i][2], L(AR[i][1], p)))  //M->LL
      img = "M->LL";
    break;
    case 17:
    ...
    break;
    case 18:
      x =  impl(AR[i][0],  M(AR[i][1], p)  , M(AR[i][2], M(AR[i][1], p)))  //M->MM
      img = "M->MM    ";
    break;
      case 19:
   ...
   case 21:
    ...
    break;
 case 22:
      x = impl(AR[i][6],   M(AR[i][5], M(AR[i][1], p)) ,     impl(AR[i][0],  M(AR[i][4], L(AR[i][3], p))    ,  impl(AR[i][2], M(AR[i][1], p), p)   )   )   //K
      img = "MMa -> MLa->(Ma->a)      i1->f2       Si ";
    break;
 case 23:
 ...
 case 24:
    ...
    break;
 case 25:
      x =  impl(AR[i][0],  L(AR[i][3], p)  , M(AR[i][2], L(AR[i][1], p)))  //M->ML
      img = "b3: L->ML    ";
    break;
 case 26:
